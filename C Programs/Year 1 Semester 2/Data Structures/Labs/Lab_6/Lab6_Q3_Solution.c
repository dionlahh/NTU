...
ListNode* HashSearch(HashTable Q3Hash, int key)
{
    ...
    if (Q3Hash.hSize != 0)
        address = Hash(key,Q3Hash.hSize);
    else
        return NULL;
    
    if (Q3Hash.Table[address].size == 0)
        return NULL;

    ptr = Q3Hash.Table[address].head;
...
int HashInsert(HashTable* Q3HashPtr, int key)
{
    ...
    if (Q3HashPtr->hSize != 0)
        address = Hash(key, Q3HashPtr->hSize);
    else
        return 0;
    ...
    if (Q3HashPtr->Table[address].size == 0)
    {
        Q3HashPtr->Table[address].head = malloc(sizeof(ListNode));
        Q3HashPtr->Table[address].head->key = key;
        Q3HashPtr->Table[address].head->next = NULL;
        Q3HashPtr->Table[address].size++;
        Q3HashPtr->nSize++;
        return 1;
    }
    else
    {
        ptr = Q3HashPtr->Table[address].head;
        Q3HashPtr->Table[address].size++;
        Q3HashPtr->nSize++;
        while (ptr->next != NULL)
            ptr = ptr->next;
