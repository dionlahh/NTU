...
int HashInsert(HashTable* Q1Ptr, int key)
{
    ...
    if (Q1Ptr->Table[address].size == 0)
    {
        Q1Ptr->Table[address].head = temp;
        Q1Ptr->Table[address].size++;
        Q1Ptr->nSize++;
        return 1;
    }
    else
    {
        ptr = Q1Ptr->Table[address].head;
        Q1Ptr->Table[address].head = temp;
        temp->next = ptr;
        ptr->pre = temp;
        Q1Ptr->Table[address].size++;
...
int HashDelete(HashTable* Q1Ptr, int key)
{
    ...
    {   
        Q1Ptr->Table[address].head = next;
        next->pre = NULL;
    }
    else
    {
        Q1Ptr->Table[address].head = NULL;
    }
    free(temp);
    Q1Ptr->nSize--;
    Q1Ptr->Table[address].size--;
